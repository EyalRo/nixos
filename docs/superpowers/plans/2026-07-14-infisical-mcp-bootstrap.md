...
### Task 2: Package the Infisical CLI and verify its flags

**Files:**
- Modify: `nixos/home/stags/default.nix:772-801` (the `home.packages` list)

- [ ] **Step 1: Add the package**

  In `home.packages` (around line 787, right before the `# MCP servers` comment), add `infisical`:
  ...
  (Only the `infisical` line and the updated comment are new — the rest is unchanged, shown for placement context.)
...
- [ ] **Step 3: Verify the CLI is on PATH and confirm the exact flags this plan relies on**

  ```bash
  infisical --version
  infisical login --help | grep -A3 universal-auth
  infisical run --help | grep -E '\-\-(projectId|env|path|token)'
  ```

  Expected: version prints `0.41.x`; `login --help` shows `--method`, `--client-id`, `--client-secret` for Universal Auth; `run --help` lists `--projectId`, `--env`, `--path`, `--token`. If any flag name differs from what's shown here, stop and adjust Task 4's `mkInfisicalRunCmd` template to match the real output before proceeding — do not guess.

- [ ] **Step 4: Commit**

  ```bash
  git add home/stags/default.nix
  git commit -m "Add infisical CLI to home packages"
  ```
...
- [ ] **Step 1: Log in as yourself**

  ```bash
  infisical login --domain=https://infisical.virtualdino.com
  ```

  Follow the browser prompt to authenticate with the admin account from Task 1, Step 1.
...
- [ ] **Step 1: Add the helper functions**
  ...
  mkInfisicalRunCmd = { path, binary, extraArgs ? "" }: ''
    ...
    exec infisical run --token="$INFISICAL_TOKEN" --projectId=${infisicalProjectId} --env=dev --path=${path} -- ${binary} ${extraArgs}
  '';
...
- [ ] **Step 1: Add the grammarly-specific helper**
  ...
  mkInfisicalGrammarlyCmd = path: ''
    ...
    infisical run --token="$INFISICAL_TOKEN" --projectId=${infisicalProjectId} --env=dev --path=${path} -- sh -c 'printf %s "$GRAMMARLY_COOKIES"' > "$COOKIES_FILE"
